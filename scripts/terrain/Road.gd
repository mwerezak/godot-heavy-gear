...
func setup(world_coords, grid_cells):
	footprint = grid_cells
	position = world_coords.unit_grid.axial_to_world(footprint.front())
	for grid_cell in footprint:
		var vertex = world_coords.unit_grid.axial_to_world(grid_cell) - position
		add_point(vertex)
